...
`,nn=class extends d{constructor(...e){super(...e),this.align=`right`,this.position=`bottom`,this.triggerOn=`click`,this.openState=!1,this._handleDialogClick=e=>{this.openState&&e.target===e.currentTarget&&(e.stopPropagation(),e.preventDefault(),this.close())},this._handleDialogClose=()=>{this.openState&&this.close()},this._handleDialogKeydown=e=>{if(this.openState){if(e.key===`ArrowDown`||e.key===`ArrowUp`){if(en(this._getActiveElement()))return;e.preventDefault();let t=this._getFocusableElements();if(t.length===0)return;let n=this._getActiveElement(),r=t.findIndex(e=>e===n);r=e.key===`ArrowDown`?r===-1?0:(r+1)%t.length:r===-1?t.length-1:(r-1+t.length)%t.length,t[r].focus()}else if(e.key===`Enter`||e.key===` `){let t=this._getActiveElement();if(e.key===` `?$t(t):en(t))return;t&&typeof t.click==`function`&&(e.preventDefault(),t.click())}}},this._closeTimeout=null,this._handleMouseEnter=()=>{this.triggerOn===`hover`&&(this._closeTimeout&&=(clearTimeout(this._closeTimeout),null),this.open())},this._handleMouseLeave=()=>{this.triggerOn===`hover`&&(this._closeTimeout&&clearTimeout(this._closeTimeout),this._closeTimeout=setTimeout(()=>{this.close(),this._closeTimeout=null},300))},this._handleResize=()=>{this.openState&&this._updatePosition()}}static{this.styles=_`
  ...
  `]}render(){return n`
    ...
    `}willUpdate(e){(e.has(`mailbox`)||e.has(`uid`))&&(this.view=null,this.notice=null,this.load())}t(e,t){return this.i18nStore?.t(e,t)??e}async load(){let e=++this.loadSeq,{mailbox:t,uid:n}=this;if(!t||!n)return;let r;try{r=await sn.fetchInvitation(t,n)}catch(t){console.error(`Failed to read the invitation`,t),e===this.loadSeq&&(this.view=null);return}if(e!==this.loadSeq||(this.view=r,!r))return;r.calendars.some(e=>e.path===this.calendarPath)||(this.calendarPath=r.copy?.calendarPath||r.calendars[0]?.path||``);let i=JSON.stringify([t,n]);r.autoApply&&!this.autoApplied.has(i)&&(this.autoApplied.add(i),await this.apply())}async respond(e){let t=this.view;if(!(!t||this.busy)){this.busy=!0,this.notice=null;try{let n=await sn.respond({mailbox:this.mailbox,uid:this.uid,status:e,calendarPath:t.copy?void 0:this.calendarPath,lang:this.i18nStore?.getLanguage?.()});this.notice=n.sendFailed?{key:`invitations.sendFailed`,tone:`error`}:{key:n.sent?`invitations.sent`:`invitations.saved`,tone:`info`}}catch(e){console.error(`Failed to answer the invitation`,e),this.notice={key:ze(e)?`invitations.changedElsewhere`:`invitations.answerFailed`,tone:`error`}}finally{this.busy=!1}await this.load()}}async apply(){let e=this.view;if(!(!e||this.busy)){this.busy=!0,this.notice=null;try{let t=(await sn.apply({mailbox:this.mailbox,uid:this.uid,calendarPath:this.calendarPath})).removed?`invitations.removed`:e.state===`add`?`invitations.added`:`invitations.updated`;this.notice={key:t,tone:`info`}}catch(e){console.error(`Failed to update the calendar`,e),this.notice={key:ze(e)?`invitations.changedElsewhere`:`invitations.applyFailed`,tone:`error`}}finally{this.busy=!1}await this.load()}}kindKey(e){switch(e.method){case`cancel`:return`cancelled`;case`reply`:return`reply`;case`request`:return e.kind===`task`?`task`:e.state===`update`?`updated`:`invitation`}return e.kind===`task`?`task`:`event`}when(e){let t=this.i18nStore?.getIntlLanguage?.()||void 0,n=new Intl.DateTimeFormat(t,{weekday:`short`,year:`numeric`,month:`short`,day:`numeric`}),r=e=>new Date(`${e.slice(0,10)}T00:00:00`);if(e.kind===`task`&&!e.start){if(!e.end)return``;let i=e.allDay?n.format(r(e.end)):new Intl.DateTimeFormat(t,{dateStyle:`medium`,timeStyle:`short`}).format(new Date(e.end));return`${this.t(`tasks.due`)}: ${i}`}if(!e.start)return``;if(e.allDay){let t=r(e.start);if(!e.end)return n.format(t);let i=r(e.end);return i.setDate(i.getDate()-1),i>t?`${n.format(t)} - ${n.format(i)}`:n.format(t)}let i=new Date(e.start),a=e.end?new Date(e.end):i,o=new Intl.DateTimeFormat(t,{weekday:`short`,year:`numeric`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}),s=o.formatRange;return a>i&&s?s.call(o,i,a):o.format(i)}renderStateLine(e){if(e.ended)return n`
            ...
            `;if(e.state===`answer`)return!e.status||e.status===`needs-action`?c:n`<div class="muted answer">${this.t(`invitations.yourAnswer`)}: ${this.t(`invitations.statuses.${ln(e.status)}`)}</div>`;let t=un[e.state];return t?n`<div class="muted state">${this.t(`invitations.states.${t}`)}</div>`:c}renderClashes(e){if(e.ended||e.kind!==`event`||e.method!==`request`||e.state!==`answer`&&e.state!==`update`||e.status===`declined`)return c;if(!e.clashes.length)return n`<div class="muted clashes">${this.t(`invitations.noConflicts`)}</div>`;let t=e.clashes.map(e=>e.summary||this.t(`calendar.noTitle`)).join(`, `);return n`<div class="clash clashes">${this.t(`invitations.conflictsWith`,{names:t})}</div>`}renderSender(e){if(e.ended||e.senderVerified||!e.sender||![`answer`,`update`,`cancel`,`reply`].includes(e.state))return c;let t=e.method===`reply`?`invitations.unverifiedReply`:`invitations.unverified`;return n`<div class="warning sender">${this.t(t,{sender:e.sender})}</div>`}renderPicker(e){return e.copy||e.calendars.length<2?c:n`
            <alps-select
                .value=${this.calendarPath}
