...
beforeEach(() => {
  ...
  vi.spyOn(messageSync, 'syncLabels').mockResolvedValue(undefined);
...
describe('creating and renaming', () => {
  it('tells a name that is taken apart from a failure', async () => {
    ...
    expect(messageSync.syncLabels).toHaveBeenCalledTimes(1);
    expect(messageSync.sync).not.toHaveBeenCalled();
  });
  ...
  it('points the poll at the new name, and re-reads only the folder list', async () => {
    // The poll asks for whatever mailbox the sync service holds, and the old
    // name no longer exists on the server.
    ...
    // A rename moves no mail, so the page of it on screen is not re-listed.
    expect(messageSync.syncLabels).toHaveBeenCalledTimes(1);
    expect(messageSync.sync).not.toHaveBeenCalled();
    expect(renamed.mock.invocationCallOrder[0]).toBeLessThan((messageSync.syncLabels as any).mock.invocationCallOrder[0]);
  });
...
describe('deleting and emptying', () => {
  it('moves the poll off a deleted folder before re-reading', async () => {
    ...
    // Deleting the folder on screen sends the view to the Inbox, so there is a
    // page of mail to read — the folder list alone would not fill it.
...
describe('subscriptions', () => {
  ...
  it('re-reads the folder list, and only for the one that took', async () => {
    fetchMock.mockResolvedValueOnce(answer(200)).mockResolvedValueOnce(answer(500));
    await ops.setSubscribed('Work', true);
    await ops.setSubscribed('Work', false);
    // Subscribing changes which folders are listed, never what is in one.
    expect(messageSync.syncLabels).toHaveBeenCalledTimes(1);
    expect(messageSync.sync).not.toHaveBeenCalled();
  });
});
