...
func (p *plugin) advertisesAutoSchedule(ctx *alps.Context, hc *http.Client, homeSet string) bool {
	req, err := http.NewRequestWithContext(ctx.Request.Context(), http.MethodOptions, davsave.URL(p.urlFor(ctx.Session.Username()), homeSet), nil)
	if err != nil {
		return false
	}
	resp, err := hc.Do(req)
...
<D:propfind xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav"><D:prop><C:calendar-user-address-set/></D:prop></D:propfind>`
	req, err := http.NewRequestWithContext(ctx.Request.Context(), "PROPFIND", davsave.URL(p.urlFor(ctx.Session.Username()), principal), strings.NewReader(body))
	if err != nil {
		return nil
	}
