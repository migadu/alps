...
func TestURLForFollowsTheLogin(t *testing.T) {
	...
	routed := "https://cal.routed.example"
	p := &plugin{url: global, srv: stubServer(routerStub{
		service: provider.ServiceCalDAV, user: "routed@example.com", url: routed,
	})}
...
func TestURLForKeepsGoingWhenTheProviderNamesNonsense(t *testing.T) {
	global, _ := url.Parse("https://global.example")
	p := &plugin{url: global, srv: stubServer(routerStub{
		service: provider.ServiceCalDAV, user: "broken@example.com", url: "://not a url",
	})}
...
func TestURLForCachesWithoutCrossingLogins(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if got := p.urlFor("routed@example.com"); got.String() != "https://cal.routed.example" {
